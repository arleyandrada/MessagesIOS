...
#define ID NSStringFromClass([JSQMessagesViewController class])
...
CGFloat currentToolbarHeight = 44.0f;
BOOL isDealloc = NO;
...
{
    //ARLEY WAS HERE...
    NSString *className = NSStringFromClass([JSQMessagesViewController class]);
...
+ (instancetype)messagesViewController
{
    ...
    NSString *path = [[[NSBundle mainBundle] resourcePath]
                      stringByAppendingPathComponent:@"modules/br.com.arlsoft.messages/"];
    NSBundle *bundle = [NSBundle bundleWithPath:path];
    return [[[self class] alloc] initWithNibName:className bundle:bundle];
...
- (void)jsq_configureMessagesViewController
{
    ...
    defaultToolbarHeight = kJSQMessagesInputToolbarHeightDefault;
    self.toolbarHeightConstraint.constant = defaultToolbarHeight;
    currentToolbarHeight = self.toolbarHeightConstraint.constant;
...
//- (void)setShowInputToolbar:(BOOL)visible
- (void)showInputToolbar:(BOOL)visible isStartup:(BOOL)startup
{
    self.inputToolbar.hidden = !visible;
...
- (void)setInputToolbarDefaultHeight:(CGFloat)height
{
    defaultToolbarHeight = height;
}
...
{
    //Loucura!!!!! Só a Apple e a Appcelerator faz isso por você!!!
    //Solução para o problema da perda do controle sobre a exibição do teclado
    //quando uma nova janela é aberta e posteriormente fechada retornando ao "chat"
...
- (void)didReceiveMemoryWarning
{
    ...
    NSLog(@"[WARN] MEMORY WARNING: %s", __PRETTY_FUNCTION__);
}
...
- (void)didPressSendButton:(UIButton *)button
    ...
    [self.delegate didPressSendButton:button
...
- (void)didPressAccessoryButton:(UIButton *)sender {
    ...
    [self.delegate didPressAccessoryButton:sender];
}
...
- (void)finishLoadEarlierMessages:(NSInteger)index
{
    ...
    dispatch_async(dispatch_get_main_queue(), ^{
        ...
        if (items > 0 && index >=0 && index < items) {
            if (index > 0){
                [self.collectionView scrollToItemAtIndexPath:[NSIndexPath indexPathForItem:index - 1 inSection:0]
...
{
    NSAssert(NO, @"ERROR: obsolete method used: %s", __PRETTY_FUNCTION__);
    return nil;
    ...
    NSString *cellIdentifier = isSystemMessage ? self.systemCellIdentifier : isOutgoingMessage ? self.outgoingCellIdentifier : self.incomingCellIdentifier;
...
- (BOOL)collectionView:(UICollectionView *)collectionView shouldShowMenuForItemAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    JSQMessagesCollectionViewCell *selectedCell = (JSQMessagesCollectionViewCell *)[self.collectionView cellForItemAtIndexPath:indexPath];
                  ...
                  layout:(JSQMessagesCollectionViewFlowLayout *)collectionViewLayout sizeForItemAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if (isSystemMessage) {
        bubbleFont = collectionViewLayout.systemMessageBubbleFont;
    } else {
        bubbleFont = collectionViewLayout.messageBubbleFont;
    }
...
- (void)collectionView:(JSQMessagesCollectionView *)collectionView didTapMessageBubbleAtIndexPath:(NSIndexPath *)indexPath {
    ...
    [self.delegate collectionView:collectionView didTapMessageBubbleAtIndexPath:indexPath];
}
                ...
                header:(JSQMessagesLoadEarlierHeaderView *)headerView didTapLoadEarlierMessagesButton:(UIButton *)sender
{
    ...
    [self.delegate collectionView:collectionView header:headerView didTapLoadEarlierMessagesButton:sender];
}
...
- (void)textViewDidBeginEditing:(UITextView *)textView
{
    ...
    [self.delegateTextView textViewDidBeginEditing:textView];
}
...
- (void)textViewDidChange:(UITextView *)textView
{
    ...
    [self.delegateTextView textViewDidChange:textView];
}
...
- (void)textViewDidEndEditing:(UITextView *)textView
{
    ...
    [self.delegateTextView textViewDidEndEditing:textView];
}
...
- (void)jsq_adjustInputToolbarHeightConstraintByDelta:(CGFloat)dy
{
    ...
    if (self.toolbarHeightConstraint.constant < defaultToolbarHeight) {
        self.toolbarHeightConstraint.constant = defaultToolbarHeight;
    }
...
- (void)jsq_updateCollectionViewInsets
{
    [self jsq_setCollectionViewInsetsTopValue:self.topLayoutGuide.length + self.topContentAdditionalInset
                                  bottomValue:CGRectGetHeight(self.collectionView.frame)
     - CGRectGetMinY(self.inputToolbar.frame)
     - (self.inputToolbar.hidden ? currentToolbarHeight : 0.0F)];
}
