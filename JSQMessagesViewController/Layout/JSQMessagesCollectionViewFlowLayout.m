...
- (void)jsq_configureFlowLayout
{
    ...
    _systemMessageBubbleFont = [UIFont boldSystemFontOfSize:12.0f];
    _messageBubbleLeftRightMargin = 40.0f;
    _messageBubbleLeftMargin = 20.0f;
    _messageBubbleRightMargin = 20.0f;
    _messageBubbleTextViewFrameInsets = UIEdgeInsetsMake(0.0f, 0.0f, 0.0f, 6.0f);
    _systemMessageBubbleTextViewFrameInsets = UIEdgeInsetsZero;
...
- (void)setSystemMessageBubbleFont:(UIFont *)systemMessageBubbleFont
{
    NSParameterAssert(systemMessageBubbleFont != nil);
    _systemMessageBubbleFont = systemMessageBubbleFont;
    [self invalidateLayoutWithContext:[JSQMessagesCollectionViewFlowLayoutInvalidationContext context]];
}
...
- (void)setMessageBubbleLeftMargin:(CGFloat)messageBubbleLeftMargin
{
    _messageBubbleLeftMargin = messageBubbleLeftMargin;
    [self invalidateLayoutWithContext:[JSQMessagesCollectionViewFlowLayoutInvalidationContext context]];
}

- (void)setMessageBubbleRightMargin:(CGFloat)messageBubbleRightMargin
{
    _messageBubbleRightMargin = messageBubbleRightMargin;
    [self invalidateLayoutWithContext:[JSQMessagesCollectionViewFlowLayoutInvalidationContext context]];
}
...
- (void)jsq_didReceiveDeviceOrientationDidChangeNotification:(NSNotification *)notification
{
    [self receiveDeviceOrientation];
}

- (void)receiveDeviceOrientation
{
    [self.dynamicAnimator removeAllBehaviors];
    [self.visibleIndexPaths removeAllObjects];
    [self invalidateLayoutWithContext:[JSQMessagesCollectionViewFlowLayoutInvalidationContext context]];
...
- (CGSize)messageBubbleSizeForItemAtIndexPath:(NSIndexPath *)indexPath messageBubbleFont:(UIFont*)bubbleFont text:(NSString*)messageText
{
...
- (void)jsq_configureMessageCellLayoutAttributes:(JSQMessagesCollectionViewLayoutAttributes *)layoutAttributes
{
    ...
    CGSize messageBubbleSize;
    
    BOOL isSystemMessage = messageData.sender == nil || [messageData.sender isEqualToString:@""];
    BOOL isOutgoingMessage = [messageData.sender isEqualToString:[self.collectionView.dataSource sender]];

    if (isSystemMessage) {
        messageBubbleSize = [self messageBubbleSizeForItemAtIndexPath:indexPath messageBubbleFont:self.systemMessageBubbleFont text:[messageData text]];
    } else {
        messageBubbleSize = [self messageBubbleSizeForItemAtIndexPath:indexPath messageBubbleFont:self.messageBubbleFont text:[messageData text]];
    }
    ...
    CGFloat leftMargin = ceilf(messageBubblePadding / 2);
    CGFloat rightMargin = messageBubblePadding - leftMargin;
    layoutAttributes.messageBubbleLeftMargin = leftMargin;
    layoutAttributes.messageBubbleRightMargin = rightMargin;
    
    if (isSystemMessage)
    {
        layoutAttributes.textViewFrameInsets = self.systemMessageBubbleTextViewFrameInsets;
    } else {
    ...
    layoutAttributes.systemMessageBubbleFont = self.systemMessageBubbleFont;
